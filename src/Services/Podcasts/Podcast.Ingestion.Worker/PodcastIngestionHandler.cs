...
public class PodcastIngestionHandler : IPodcastIngestionHandler
{
    private static readonly string[] AcceptedTopics =
    {
        ".NET", "C#", "F#", "ASP.NET", 
        "MAUI", "Xamarin", "Blazor", 
        "Microservices", "Visual Basic", "Visual Studio"
    };
    ...
    public PodcastIngestionHandler(
        PodcastDbContext podcastDbContext, IFeedClient feedClient, ILogger<PodcastIngestionHandler> logger)
    {
        ...
        CancellationToken stoppingToken)
    {
        ...
        _logger.LogInformation(
            "The show {Title} doesn't already exist in the database, checking for accepted topics in the category list.",
            title);
        ...
        {
            _logger.LogInformation(
                "The show {Title} at {Url} was not automatically approved.",
                title, url);
            ...
            _logger.LogInformation(
                "The show {Title} at {Url} was saved as a user-submitted feed.",
                title, url);
